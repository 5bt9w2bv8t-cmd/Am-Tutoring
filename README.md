...
1. Create a Supabase project. In **SQL Editor**, run all of `schema.sql`. It is safe to rerun on an existing ClassMatch database. If the database was created from an older schema, run `migrations/20260918_hardening.sql` after it so students only see open availability while assigned tutors can see their own booked history.
...
5. Copy `.streamlit/secrets.toml.example` to `.streamlit/secrets.toml`, replace every placeholder, and generate a unique 32+ character `COOKIE_PASSWORD`. Set `OWNER_EMAILS` to the comma-separated verified owner emails. Never commit this file.
6. Create owner accounts with `taleenalali5@gmail.com` and `mazharysaleh@gmail.com`. Both receive access to the owner dashboard only when those exact addresses are present in `OWNER_EMAILS`.
