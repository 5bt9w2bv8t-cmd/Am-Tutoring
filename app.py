...
def home(user: dict | None) -> None:
    st.markdown('<p class="eyebrow">STUDENT-LED · ONLINE · FREE</p>', unsafe_allow_html=True)
    st.title("Students helping students grow.")
    st.markdown('<p class="lead">AM Tutoring connects students in Syria and the UAE with approved student tutors aged 13–18.</p>', unsafe_allow_html=True)
    if st.button("Find a tutor", type="primary", use_container_width=False):
        go("Find a tutor")
        st.rerun()

    st.markdown("### How it works")
    columns = st.columns(3)
    for column, number, title, copy in zip(columns, ("01", "02", "03"), ("Choose what you need", "Pick an open time", "The owner confirms"), ("Select country, grade, and subject.", "Compare approved tutors and dated slots.", "Everyone receives the status and lesson link by email.")):
        with column:
            st.markdown(f'<div class="step"><span>{number}</span><h3>{title}</h3><p>{copy}</p></div>', unsafe_allow_html=True)

    st.markdown("### About AM Tutoring")
    st.write("We make friendly, free academic help easier to access while giving student tutors a safe way to share what they know. The owner approves every tutor, controls availability, and confirms each session.")
    st.info("Safety: enter only a student’s first name. Do not add surnames, phone numbers, home or school addresses, ID numbers, or medical information.")
    st.divider()
    st.markdown("### Want to tutor?")
    st.write("AM Tutoring does not collect tutor applications on this website. Email your CV, age, grade, subjects, languages, and usual availability to the owner.")
    st.link_button("Email taleenalali5@gmail.com", "mailto:taleenalali5@gmail.com?subject=AM%20Tutoring%20-%20Tutor%20interest")


def find_tutor(user: dict | None) -> None:
    st.header("Find a tutor")
    st.caption("Choose your learning needs, then select a tutor and a real available time.")
    ...
    tutor_id = st.selectbox("Choose a tutor", list(labels), format_func=labels.get)
    ...
    if unavailable:
        with st.expander(f"Filled or pending times ({len(unavailable)})"):
            for slot in unavailable:
                st.write(f"{format_slot(slot)} — {'Pending' if slot['status'] == 'requested' else 'Filled'}")
    if not available:
        st.info("All upcoming times are filled or waiting for confirmation.")
        return
    if not user:
        st.warning("A parent or guardian must sign in before requesting a lesson.")
        if st.button("Sign in or create an account"):
            go("My account")
            st.rerun()
        return
    slot_labels = {slot["id"]: format_slot(slot) for slot in available}
    with st.form("session_request", clear_on_submit=True):
        slot_id = st.selectbox("Available time", list(slot_labels), format_func=slot_labels.get)
        ...
        notes = st.text_area("What help is needed?", max_chars=1000)
        consent = st.checkbox("I am the parent/guardian or have their permission, and I agree to receive session emails.")
        submitted = st.form_submit_button("Request this session", type="primary", use_container_width=True)
    if submitted:
        try:
            ...
            flash("success", "Request saved and email copies sent." if emailed else "Request saved. You can track it in My account; email delivery needs owner setup or review.")
