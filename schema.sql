...
with check (availability_slots.status = 'open' and exists (select 1 from public.tutor_roles r where r.tutor_id = availability_slots.tutor_id and r.email = lower(coalesce(auth.jwt() ->> 'email', ''))));
...
with check (availability_slots.status in ('open', 'cancelled') and exists (select 1 from public.tutor_roles r where r.tutor_id = availability_slots.tutor_id and r.email = lower(coalesce(auth.jwt() ->> 'email', ''))));
